...
function processarFormulario() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $id = isset($_POST['id-post']) ? ($_POST['id-post']) : '';
        ...
        $dados = array('id' => $id ,'titulo' => $titulo , 'resumo' => $resumo, 'imagen_url' => '/img/'.$nomeArquivo, 'data' => $dataAtual);

        return $dados;
    }
}

$dadosForm = processarFormulario();
...
$banco->AtualizarDados(
    $dadosForm["id"],
    (!empty($dadosForm["titulo"])) ? $dadosForm["titulo"] : "", 
    (!empty($dadosForm["resumo"])) ? $dadosForm["resumo"] : "", 
    (!empty($dadosForm["imagen_url"])) ? $dadosForm["imagen_url"] : ""
);
